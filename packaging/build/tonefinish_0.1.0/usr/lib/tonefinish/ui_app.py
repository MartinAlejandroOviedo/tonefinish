...
if TYPE_CHECKING:
    ...
    try:
        ...
        QCheckBox = QComboBox = QDoubleSpinBox = QFileDialog = QLabel = QLineEdit = QPlainTextEdit = QPushButton = QSpacerItem = QTabWidget = QTableWidget = QTableWidgetItem = _QtWidgetPlaceholder  # type: ignore
        ...
        def run(self) -> None:
            try:
                ...
                for idx, audio_path in enumerate(files, start=1):
                    self.progress.emit(f"Procesando {idx}/{len(files)}: {audio_path.name}")
                ...
                self.finished.emit(f"Lote completado: {processed} archivos.")
            except Exception as exc:
                self.error.emit(str(exc))
    ...
    class MainWindow(QWidget):
        ...
        def _start_worker(self, worker: QObject, on_finished) -> None:
            ...
            if hasattr(worker, "progress"):
                worker.progress.connect(self.append_log)  # type: ignore[attr-defined]
        ...
        def _handle_batch_finished(self, message: str) -> None:
